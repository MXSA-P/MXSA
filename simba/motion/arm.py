...
class ArmController:
    ...
    def move_to_xyz(self, x, y, z):
        """move arm end-effector to 3D coordinates using inverse kinematics.

        assumes origin (0,0,0) is the base of the arm.
        x: left/right (cm)
        y: forward distance (cm)
        z: height (cm)
        ...
        wrist_angle = self.home_angles["wrist"] + math.degrees(theta2_rad)

        log_event(
            "motion", f"IK calculated: rot={
                rot_angle:.1f}, elbow={
                elbow_angle:.1f}, wrist={
                wrist_angle:.1f} for xyz({x},{y},{z})")

        # move arm smoothly to calculated angles (lock elbow_2 straight)
        self.move_smooth({
            "rotation": rot_angle,
            "elbow": elbow_angle,
            "elbow_2": 90.0,
