...
def api_diagnostics_run():
    ...
    if test_name == "arm_sweep":
        ...
        try:
            ...
            return jsonify({
                "status": "path recorder ok",
                "step_count": pr.step_count,
                "is_returning": pr.is_returning,
            })
        except Exception as e:
            return jsonify({"error": f"path recorder test failed: {e}"}), 500

    elif test_name == "ollama_llm":
        try:
            import requests as req
            url = getattr(_brain, 'ollama_url', 'http://localhost:11434/api/chat')
            ...
            if resp.status_code == 200:
                ...
                return jsonify({
                    ...
                    "models": model_names,
                    "active_model": getattr(_brain, 'ollama_model', 'unknown'),
                })
            else:
                return jsonify({"error": f"ollama returned {resp.status_code}"}), 500
        except Exception as e:
            return jsonify({"error": f"ollama test failed: {e}"}), 500
...
def api_arm_home():
    ...
    try:
        _brain.arm.home()
        return jsonify({"status": "arm homed"})
    except Exception as e:
        return jsonify({"error": f"arm error: {e}"}), 500
    ...
    try:
        if grip is not None:
            ...
            if grip == "open":
                ...
                return jsonify({"error": "unsupported grip"}), 400
...
def handle_command_ws(data):
    ...
    if _brain is not None:
        try:
            if hasattr(_brain, "_on_voice_text"):
                _brain._on_voice_text(command)
        except Exception as exc:
...
@auth.login_required
def api_path():
    """return the path recorder status as json."""
    if _brain is not None:
        try:
            return jsonify(_brain.path_recorder.to_dict())
        except Exception as exc:
            logger.error(f"path api error: {exc}")
